...
let bfs_levelAsync maxSubtasks graph startVertices =
    let rec inner level (frontier: Vector.SparseVector<_>) (visited: Vector.SparseVector<_>) =
        if frontier.nvals > 0UL<nvals> then
            let op_add x y =
                match (x, y) with
            ...
            let new_frontier = LinearAlgebra.vxmAsync maxSubtasks op_add op_mult frontier graph

            match new_frontier with
            | Result.Failure(e) -> Result.Failure(NewFrontierCalculationProblem(e))
            | Result.Success(new_frontier) ->
                let frontier = Vector.maskAsync maxSubtasks new_frontier visited (fun x -> x.IsNone)
                ...
                | Result.Success(frontier) ->
                    ...
                    let visited = Vector.map2Async maxSubtasks visited new_frontier op_add
