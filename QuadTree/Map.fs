...
let private rotateRight tree =
    ...
    | Node(x, vx, _, left, right) ->
        ...
        | Node(y, vy, _, ly, ry) ->
            Node(y, vy, 1 + max (height ly) (height (makeNode x vx ry right)), ly, makeNode x vx ry right)
...
let private rotateLeft tree =
    ...
    | Node(x, vx, _, left, right) ->
        ...
        | Node(y, vy, _, ly, ry) ->
            Node(y, vy, 1 + max (height (makeNode x vx left ly)) (height ry), makeNode x vx left ly, ry)
...
let private balance k v l r =
    ...
    if hl - hr > 1 then
        match l with
        | Node(lk, lv, _, ll, lr) when height ll >= height lr -> rotateRight (Node(k, v, 0, l, r))
        | Node(lk, lv, _, ll, lr) ->
        ...
        match r with
        | Node(rk, rv, _, rl, rr) when height rr >= height rl -> rotateLeft (Node(k, v, 0, l, r))
        | Node(rk, rv, _, rl, rr) ->
...
let toList map =
    fold (fun acc k v -> (k, v) :: acc) [] map |> List.rev
