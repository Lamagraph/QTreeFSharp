...
let sssp graph (startVertex: uint64) =
    ...
    let rec inner (frontier: Vector.SparseVector<_>) (visited: Vector.SparseVector<_>) iter_num =
        if frontier.nvals > 0UL<nvals> && iter_num <= int frontier.length then
            resultM {
                let! new_frontier = LinearAlgebra.vxm op_add op_mult frontier graph |> Result.mapError NewFrontierCalculationProblem
                ...
                let! frontier = Vector.map2 new_frontier visited op_min |> Result.mapError FrontierCalculationProblem
